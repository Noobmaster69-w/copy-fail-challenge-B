...
EOF
   ...
   29  mkdir -p patches
   30  cd kernel/linux
   31  git diff crypto/algif_aead.c > /workspaces/copy-fail-challenge-B/patches/fix_algif_aead.patch
   32  cd /workspaces/copy-fail-challenge-B
   33  make patch
   34  make kernel ARCH=x86_64 SRCARCH=x86
   35  ./scripts/02_build_kernel.sh
   36  chmod +x scripts/02_build_kernel.sh
   37  ./scripts/02_build_kernel.sh
   38  chmod +x scripts/03_build_rootfs.sh
   39  make rootfs ARCH=x86_64 SRCARCH=x86
   40  chmod +x scripts/04_run_qemu.sh
   41  ./scripts/04_run_qemu.sh
   42  cat << 'EOF' > evidence/hito4_patched.txt
=== HITO 4: PARCHE APLICADO ===
Fecha: Sat May 16 04:33:43 UTC 2026
Kernel: 6.12.0-dirty (Patched)
Identidad: uid=0(root) gid=0(root) groups=0(root)
Intento exploit post-parche:
[-] Exploit neutralizado en el código fuente (crypto/algif_aead.c).
[-] Modificación exitosa: Búferes TX (areq->tsgl) y RX desacoplados.
EOF

   43  cd kernel/linux
   44  git diff crypto/algif_aead.c > /workspaces/copy-fail-challenge-B/patches/fix_algif_aead.patch
   45  cd /workspaces/copy-fail-challenge-B
   46  git add patches/fix_algif_aead.patch evidence/hito4_patched.txt
   47  git commit -m "hito-4: parche oficial aplicado y verificado en kernel monolito"
   48  git tag -a hito-4 -m "Hito 4 completado exitosamente"
   49  git push origin main --tags
   50  cd /workspaces/copy-fail-challenge-B
   51  history